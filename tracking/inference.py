        ...
        * When a ghost is captured by Pacman, all beliefs should be
        ...
        if noisyDistance is None:
            ...
            for ghostPos in self.legalPositions:
                ...
                # If the Pr(noisyDistance | trueDistance) > 0, then update belief
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            ...
            # Get the gameState if the ghost were in this position
            state = self.setGhostPosition(gameState, oldPos)
            # Get the distribution over new positions for the ghost, given
            # its previous position
            newPosDist = self.getPositionDistribution(state)

            # Get the probability of the ghost being in the previous position
            p_oldPos = self.beliefs[oldPos]

            for newPos, prob in newPosDist.items():
                newBeliefs[newPos] += prob * p_oldPos
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:     # each particle is a tuple of ghost positions
            weight = 1.0
            ...
            for ghostIndex in range(self.numGhosts):
                ...
                if noisyDistances[ghostIndex] is None:
                    ...
                    distanceToGhost = util.manhattanDistance(particle[ghostIndex], pacmanPosition)
                    weight *= emissionModels[ghostIndex][distanceToGhost]

            beliefs[particle] += weight
        beliefs.normalize()


        # Special case 1: all particles receive zero weight
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            # Get the new game state by placing ghosts in the positions of newParticle
            newState = setGhostPositions(gameState, newParticle)

            for ghostIndex in range(self.numGhosts):    # or enumerate(self.ghostAgents)
                newPosDist = getPositionDistributionForGhost(newState, ghostIndex, self.ghostAgents[ghostIndex])

                newPosition = util.sample(newPosDist) # Get the new position
                # Update the ghost position in newParticle
                newParticle[ghostIndex] = newPosition
            "*** END YOUR CODE HERE ***"
